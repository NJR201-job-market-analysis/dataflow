...
from dataflow.constant import DOCKER_IMAGE, DOCKER_NETWORK_MODE


# 建立一個 DockerOperator 任務的函式，回傳一個 Airflow 的任務實例
def create_producer_1111_task() -> DockerOperator:
    return DockerOperator(
        # 設定這個 task 在 DAG 中的名稱（唯一識別碼）
        task_id="producer_1111",
        image=DOCKER_IMAGE,
        command="pipenv run python -m crawlers.1111.producer",
